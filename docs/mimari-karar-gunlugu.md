...
- **DÜRÜST SINIRLAR:** (a) Özel alan adı yok; kanonik varsayılan `whenly-site.vercel.app` — alan alınınca `SITE_URL` Variable güncellenir (tek nokta). (b) Vercel projesi + BWT/GSC kaydı + G2/Capterra/AlternativeTo/PH listelemeleri **insan işi** — playbook §3'te, yapılmadan "yapıldı" denmez. (c) llms.txt bugün sağlayıcılarca okunmuyor, schema kanıtı zayıf — bilinçli düşük-maliyet hedge. (d) Sitede sahte sosyal kanıt/kullanıcı sayısı YOK; istatistikler saha araştırmamızdan. (e) GA4/analitik site'ye eklenmedi (ölçüm playbook'ta; izleme eklenirse gizlilik metni güncellenmeli). (f) Zaman beklentisi: Perplexity haftalar, ChatGPT 6-12 hafta, eğitim-verisi aylar — garanti yok.
