...
- **AÇIK EKSİK (dürüst):** Render backend env'inde hâlâ `SUPABASE_URL`/`SERVICE_ROLE_KEY` YOK → backend in-memory çalışıyor, bu DB'yi henüz KULLANMIYOR. Şema hazır; bağlantı için Render'a gizli anahtarların eklenmesi gerekiyor (kullanıcıda).
- **ISO/TOGAF:** 25012 (veri şeması bütünlüğü/güncellik — canlı senkron) · 27002 (erişim kontrol: RLS sahibi-only) · TOGAF Phase F (Migration) + Phase G (governance kaydı), sınıf: Basitleştirme (mevcut migration'ların uygulanması; yeni mimari yok).
