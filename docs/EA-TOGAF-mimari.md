...
| C-045 | **RAG bilgi tabanı — pgvector deposu (M3 / FAZ 3.1)** — migration 0020 (vector ext + `embeddings` vector(768) + HNSW cosine + `match_embeddings` RPC + RLS service-role); domain `RagStore` (upsert/query/available) + `SupabaseRagStore`/`NullRagStore` + `indexDocuments` embed-yazım yolu + container `rag` + database.types. Boyut 768 (sağlayıcı-uyumlu) | Artımlı `[M]` | RAG depolama temeli; M3.2 korpus + M3.3 retrieval için zemin | P5✓(RLS kapalı-varsayılan, service-role)·P8✓(port/adapter; **220 test 42/42**; typecheck/biome)·P9✓(migration İZİNLİ ama bu ortamda token yok→CANLI UYGULANMADI, kullanıcı uygular; kod dormant-safe, çağıran yok; pgvector adapteri canlı-DB-only — dürüstçe) | ADR-143 | TA-2 · kod canlı, **migration bekliyor** |
