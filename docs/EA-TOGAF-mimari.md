...
- **Dashboard (Next.js/Vercel)** — web yönetim: hesap, watcher genel görünüm, faturalama, (ileride) admin/observability.
...
> **Re-baseline notu:** C-001/002 TA-2 (Lansman) kapsamındaki kullanıcı-dönük olgunlaşmadır; mimari prensiplerde sapma yok (dispensation gerekmedi). C-005 ("Okundu" durumu) **kod+migration olarak hazır** ama `deliveries.read_at` migration'ı (0005) canlıya uygulanmadan main'e merge edilmiyor (deploy-güvenliği; CLAUDE.md migration-izni kuralı). Anahtar/izin gelince uygulanıp merge edilecek.
