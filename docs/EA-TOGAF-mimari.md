...
| C-011 | **Tanıtım sitesi + GEO mimarisi** — `apps/website` sıfır-bağımlılık SSG (36 sayfa TR+EN: 12×2 use-case + karşılaştırma + hakkında + hukuki); robots 14 AI botuna açık + sitemap(hreflang) + llms.txt + JSON-LD + IndexNow otomasyonu; 5 kollu derin araştırma sentezi `docs/GEO-pazarlama-mimarisi.md` (kanonik playbook); deploy.yml `site` job'u (Variable tanımlanınca ayrı Vercel projesi) | Artımlı | **Yeni iş yeteneği:** AI-asistan-aracılı edinim (Phase B) + yeni App bileşeni (§4.2.1) — backend'e bağımlılık YOK | P1✓(sıfır PII/sıfır API — tamamen statik)·P2✓(dokunulmadı)·P3✓(buy>build: Vercel statik; üretici ~300 satır, framework reddi gerekçeli)·P4✓(contracts ilgisiz — sınır yok)·P5✓(güvenlik başlıkları; secret yok)·P7✓(içerik verisi framework-bağımsız, Astro'ya taşınabilir)·P8✓(Func.Suit.+Perf+Interaction; 14 testle)·P9✓(sıfır bağımlılık = sağ-boyut) | ADR-090 | TA-2 · **Vercel projesi + BWT/GSC + G2/Capterra listelemeleri insan-aksiyonu bekliyor (playbook §3)** |
