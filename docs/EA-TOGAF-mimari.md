...
| C-016 | **Whenly Console derinleştirme (planlı, 4 fazlı)** — Faz A: kullanıcı e-posta araması + 360° detay drill-down (`getUserDetail` portu, `/admin/users/{id}`); Faz B: operasyon/sağlık panosu (`getOps`, `/admin/ops` — check-run/teslimat/token kanal kırılımı); Faz C: gelir & büyüme (`getGrowth`, `/admin/growth` — kayıt trendi+huni+churn+anlık MRR) + CSV dışa aktarım (`/admin/export/{users,subscriptions}.csv`). Tümü mevcut port-adapter + react-query + contracts üzerine; yeni tablo YOK | Artımlı | Yönetim App yeteneği "okuma panosu"ndan **işletme kontrol merkezi**ne derinleşir; her faz ayrı ADR + ayrı deploy | P1✓(salt-okuma; PII service-role)·P2✓(gizlilik zonu değişmedi)·P4✓(contracts genişledi, geri-uyumlu)·P5✓(hepsi `adminMiddleware` arkasında; CSV escape)·P7✓(yeni ekranlar, geri-alınabilir)·P8✓(Func.Suit.+Reliability; testle)·P9✓(snapshot/erteleme dürüstçe işaretli — uydurma trend yok) | ADR-101/102/103 | TA-2 · Faz A-C canlı · Faz D → C-017 |
| C-017 | **Whenly Console Faz D: Etkileşim & moderasyon** — yeni `ModerationRepository` (isBanned/setBanned/writeAudit/listAudit) + Supabase/in-memory; **ban enforcement** `ban.middleware` (auth sonrası `/v1/*`, fail-open, admin banlanamaz); **push yayın** `POST /admin/broadcast` (segment all/free/pro → `segmentTokens` → FCM fan-out; FCM yoksa dürüst `inactive`); **denetim günlüğü** `admin_audit` (immutable, seçili mutasyonlar `audit()` yazar) + `GET /admin/audit`. **Yeni Data varlığı:** migration **0016** (`admin_audit` + `profiles.banned`) | Artımlı | Yönetim "kontrol merkezi"ne moderasyon+iletişim+denetlenebilirlik eklenir; gizlilik zonları korunur | P1✓(audit'te PII/secret yok; FCM token server-side)·P2✓(PII zon değişmedi)·P4✓(contracts genişledi)·P5✓(adminMiddleware; ban enforcement; immutable audit RLS service-role; admin/ban durum ayrımı=öz-kilitlenme yok)·P7✓(geri-alınabilir; unban var)·P8✓(Func.Suit.+Security+Reliability; 5 test + /security-review 6/6 + /code-review 7/7)·P9✓(broadcast partileme/heartbeat bilinçli ertelendi) | ADR-104 | TA-2 · **migration 0016 açık izinle; sonra deploy** |
| C-018 | **Telegram + WhatsApp kanal bağlama** — kanal kodu/`user_channels` zaten hazırdı; bağlama: Telegram = yalnız `TELEGRAM_BOT_TOKEN` (Render config, kod yok; chat_id kullanıcı yapıştırır). WhatsApp = `WhatsAppSender`'a **onaylı-şablon** (`type:"template"`, 2 param) desteği + `WHATSAPP_TEMPLATE_NAME/LANG` env; pencere-dışı uyarı için şart. Kurulum (Meta Business+app+numara+kalıcı token+şablon / BotFather) DEPLOY §12 rehberi | Artımlı | Bildirim yeteneği FCM-ötesi (Telegram/WhatsApp) genişler; ADR-084 dikeyini üretime taşır | P1✓(PII telefon/chat_id `user_channels` RLS service-role; egress yalnız kullanıcının kendi hedefi)·P3✓(buy: Telegram/Meta API)·P4✓(env opsiyonel; geri-uyumlu)·P5✓(token Render secret; şablon enjeksiyon değil sabit param)·P7✓(env boşsa kanal kurulmaz)·P8✓(Func.Suit.+Interop; 3 test)·P9✓(Telegram oto-link/webhook bilinçli ertelendi) | ADR-106 | TA-2 · kod canlı (pasif) · **token'lar girilince aktif; WhatsApp Meta onayı bekler** |
| C-019 | **Admin kanal aç/kapa + kullanıcı "kapalı" uyarısı** — `app_settings['channels.availability']` (SettingsRepository); admin `GET/PUT /admin/channel-config` (audited) + kullanıcı `GET /config`; **teslim enforcement**: `dispatchEventDeliveries` `enabledChannels` ile süzer → kapalı kanala HİÇ gönderilmez; mobil admin/channels.tsx + channels.tsx kilitli kart + `disabledByAdmin` (×11 dil) | Artımlı | Operatöre kanal-düzeyinde işletme kontrolü; yanıltıcı "açık ama çalışmıyor" durumu önlenir | P1✓(salt admin ayar; PII yok)·P4✓(contracts genişledi; varsayılan açık=geri-uyumlu)·P5✓(adminMiddleware + audit; kullanıcı /config salt-okur)·P7✓(geri-alınabilir toggle)·P8✓(Func.Suit.+Usability; 3 test incl. teslim-enforcement)·P9✓(C/D/E ayrı increment, dürüstçe işaretli) | ADR-107 | TA-2 · canlı |
| C-020 | **LLM e-posta besteci + admin istem** — `SwitchableEmailComposer` (chatWithActive yeniden kullanır) ham uyarıyı profesyonel e-postaya (`{subject,body}`) çevirir; yalnız e-posta kanalı; LLM/parse hatasında HAM metne düşer. Admin `GET/PUT /admin/email-prompt` (app_settings `email.compose`); kaliteli varsayılan istem + "varsayılan" toggle (açıkken kutuda görünür); mobil admin/email-prompt.tsx | Artımlı | E-posta kalitesi LLM ile yükselir; admin metin kontrolü (DÜRÜST: maliyet/gecikme, fallback'li) | P1✓(yalnız uyarı metni LLM'e)·P3✓(buy: LLM)·P4✓(contracts genişledi)·P5✓(admin-guard+audit; fallback)·P7✓(varsayılana dön)·P8✓(Func.Suit.+Reliability; 4 test incl. fallback)·P9✓(yalnız email; telegram/whatsapp ham) | ADR-109 | TA-2 · canlı |
| C-021 | **Sıklık slider (D) + asistan arama-planı (E)** — sihirbazda sürüklenen `FreqSlider` (RN responder olayları, log ölçek, plan-min'i altına inmez, a11y adjustable); asistan `AssistReply`'a `searchQuery`/`searchMethods`/`feasibility` (opsiyonel) + ASSISTANT_SYSTEM istemi + wizard "Arama planı" paneli (×11 dil) → kullanıcı sorgu/yöntem/fizibiliteyi görüp onaylar | Artımlı | Kullanıcı kontrolü (sıklık) + şeffaflık/güven (arama planı); etkileşim olgunlaşır | P1✓(asistana yalnız niyet metni; PII-sınırı korunur)·P4✓(contracts opsiyonel genişledi; geri-uyumlu)·P5✓(salt UI/asistan; yeni egress yok)·P7✓(presetler korundu; opsiyonel alanlar)·P8✓(Usability+Interaction; mevcut testler korundu)·P9✓(LLM yoksa panel görünmez; slider niceMinutes'e oturur) | ADR-110 | TA-2 · canlı (görsel kabul cihazda) |
| C-022 | **Sihirbaz saf-LLM sohbeti + kullanıcı AI kişiselleştirme** — (ADR-112) hazır öneri çipleri kaldırıldı → saf AI sohbeti; (ADR-113) `profiles.ai_about`/`ai_attention` (migration 0017) + `AiProfileRepository` + `GET/PUT /me/ai-profile` + asistan istemine kullanıcı bağlamı enjeksiyonu (yalnız kendi çağrısı) + mobil ai-profile.tsx + Ayarlar satırı + i18n ×11 | Artımlı (113) + Basitleştirme (112) | Kişiselleştirilmiş, sade AI deneyimi; kullanıcı kontrolü | P1✓(PII yalnız kendi asistan çağrısına; paylaşılan topic'e gitmez)·P2✓(gizlilik zonu korundu; profiles RLS)·P4✓(contracts genişledi)·P5✓(auth'lu kendi profili; RLS)·P7✓(boş varsayılan; geri-alınabilir)·P8✓(Func.Suit.+Usability; 3 test)·P9✓(enjeksiyon yalnız asistan, tespit değil — bilinçli sınır) | ADR-112/113 | TA-2 · 112 canlı · **113 migration 0017 izinli, deploy** |
| C-023 | **Görünüm ayarları: vurgu rengi + hareket tercihi** — 5 küratörlü vurgu rengi (açık/koyu üçlü + marka gradyanı; `useTheme()` palete enjekte → className+stil-prop+gradyan tek kaynak); 6 imza yüzeyi `GRADIENT`→`useTheme().gradient`; hareket tercihi (sistem/tam/azalt) `useReduceMotion`'ı uygulama-içi ezer; yeni `appearance.tsx` (canlı önizleme + tema/renk/hareket) + i18n ×11. **Yazı boyutu + yoğunluk bilinçli ERTELENDİ** (sabit-piksel util → uygulama-çapı token göçü gerekir; hack regresyon riski) | Artımlı | Görünüm yeteneği tema-modundan kişiselleştirmeye derinleşir; kalite için yarım özellik shiplenmedi | P1✓(yalnız yerel tercih; PII/egress yok)·P4✓(GRADIENT→hook; varsayılan=çivit, geri-uyumlu)·P5✓(salt UI; yeni yüzey yok)·P7✓(varsayılana dönülebilir; kalıcı tercih)·P8✓(Usability+Portability; WCAG kontrast korundu)·P9✓(yazı/yoğunluk "yapıldı" denmeden gerekçeyle ertelendi — DÜRÜSTLÜK) | ADR-114 | TA-2 · mobil-only; migration yok · deploy |
| C-024 | **Asistan dayanıklılığı + zekâsı** — (ADR-118) LLM hatasında route sezgisel fallback (503→200) + 25s timeout; (ADR-119) Groq çapraz-sağlayıcı fallback (deepseek tökezlerse gerçek 2. LLM) + saçma/istek-değil reddi (LLM RULE #2 + sezgisel `looksUnmonitorable`) | Düzeltici | Niyet asistanı geçici LLM hatasında tıkanmaz + saçmadan junk watch üretmez; canlı LLM ile doğrulandı | P1✓(asistana yalnız niyet metni)·P5✓(yeni egress yok; mevcut sağlayıcılar)·P7✓(fallback geri-alınabilir)·P8✓(Güvenilirlik+İşlevsel Doğruluk; 161 test + canlı LLM)·P9✓(Groq-fallback canlı doğrulandı, birim-test değil — dürüstçe) | ADR-118/119 | TA-2 · canlı |
| C-025 | **Ajan geliştirme skill'i (whenly-brain)** — obsidian-mind/skills/second-brain `~/agent-refs`'e klonlandı (referans); yöntemi `.claude/skills/whenly-brain/SKILL.md`'ye uyarlandı: ADR/EA kalıcı hafıza (self-rewriting) + olay-bazlı araştırma/fizibilite/site-izni ajan playbook'u | Artımlı | Ajan oturum-bağımsız daha yetenekli; tutarlı ADR/EA + fizibilite metodolojisi | P8✓(Bakımkolaylığı; ramp-up)·P9✓(Obsidian repoları literal kurulamaz — dürüstçe yöntem uyarlandı) | ADR-120 | TA-2 · dev-tooling (deploy etkisi yok) |
| C-026 | **Model-routing ayrımı (Faz 1)** — `FixedModelSource`: niyet asistanı/ajan SABİT deepseek-v4-pro; reasoner/verifier/e-posta admin-seçili `LlmModelRouter`'da kalır. deepseek yoksa graceful router fallback | Artımlı | Asistan kalitesi sabitlenir (v4-pro); watcher maliyeti admin kontrolünde kalır | P4✓(geri-uyumlu; anahtar yoksa eski davranış)·P5✓(yeni egress yok)·P8✓(İşlevsel+Performans; 163 test)·P9✓(v4-pro maliyet/gecikme dürüstçe not edildi) | ADR-121 | TA-2 · canlı |
| C-027 | **Function-calling çekirdeği (Faz 2)** — domain agent portları + `openaiToolChat` (tools/tool_calls) + `SwitchableAgentChat` (v4-pro, Groq fallback) + `runAgent` döngüsü + araç fabrikaları (web_search/resolve_authority gerçek; site-policy/rag yer-tutucu). Dormant (Faz 4'te bağlanır) | Artımlı | Asistan ajanlaşmasının altyapısı; gerçek araç-çağıran LLM | P4✓(dormant; davranış değişmez)·P5✓(yeni egress yok; mevcut sağlayıcılar)·P8✓(İşlevsel+Bakımkolaylığı; 169 test)·P9✓(v4-pro tool desteği canlı doğrulaması beklemede — Groq kesin) | ADR-122 | TA-2 · canlı (dormant) |
| C-028 | **Asistan güvenilirliği (parse-fallback + sezgisel sertleştirme)** — `chatWithActiveJson`: çağrı+şema-doğrulama tek birim → v4-pro parse-hatasında Groq fallback (eskiden atlanıp dumb sezgisele düşüyordu); sezgisel yalnız SON mesaj + soru/asistan-hakkında reddi | Düzeltici | Asistan junk niyet üretmez; LLM hatasında gerçek 2. LLM'e düşer | P8✓(Güvenilirlik+Doğruluk; 170 test)·P9✓(sezgisel hâlâ sezgisel; gerçek araştırma Faz 4 — dürüstçe) | ADR-126 | TA-2 · canlı |
| C-029 | **Switchable embedding sağlayıcısı (Gemini ücretsiz / OpenAI)** — domain port + 2 adapter + `EmbeddingRouter` (admin-seçilebilir, app_settings) + `SwitchableEmbedder` + `GET/PUT /admin/embeddings` + mobil admin picker. Groq embedding sunmaz (araştırıldı); Gemini text-embedding-004 ücretsiz varsayılan; 768-normalize | Artımlı | RAG için gömme altyapısı; env+admin'den geçiş; ücretsiz varsayılan | P3✓(buy: Gemini/OpenAI; Gemini free)·P5✓(anahtar env secret; anahtarsız seçilemez)·P8✓(İşlevsel+Interop; 176 test)·P9✓(dormant; RAG depolama Faz 5 migration-izinli; sağlayıcı-değişimi yeniden-gömme dürüstçe not edildi) | ADR-127 | TA-2 · canlı (dormant) |
| C-030 | **Site-izni / robots.txt denetçisi (Faz 3)** — domain `SitePolicyResolver` + `RobotsSitePolicyResolver` (SSRF guard yeniden, robots.txt çek+ayrıştır, tam-blok tespiti, 24s cache) + gerçek `check_site_policy` aracı + `container.sitePolicy`. Dormant (Faz 4 ajanı kullanır) | Artımlı | 'Bu siteyi izlemek serbest mi' fizibilite girdisi | P1✓(yalnız kamusal HTTPS)·P5✓(SSRF guard origin.ts'ten)·P8✓(İşlevsel+Güvenlik; 187 test)·P9✓(robots advisory; ToS ertelendi — dürüstçe) | ADR-128 | TA-2 · canlı (dormant) |
| C-031 | **Olay-bazlı fizibilite ajanı (Faz 4 — GÖRÜNÜR)** — `AgenticIntentAssistant`: niyet asistanı artık `runAgent` ile araçları (web_search→resolve_authority→check_site_policy) GERÇEKTEN çağırır; yapısal `feasibilityVerdict` (can/partial/cannot) + `plannedSteps` + `sitePermission` döner. `AGENT_FEASIBILITY_SYSTEM` (ortak kurallar DRY parçalandı, ASSISTANT_SYSTEM byte-aynı). Arama checker'dan ayrıldı (paylaşımlı). Mobil `FeasibilityCard` (rozet+adımlar+izin) + i18n ×11. Fallback: ajan→tek-atış→sezgisel | Artımlı | Asistan tahmin yerine ARAŞTIRIR; kullanıcı 'yapabilirim/kısmen/yapamam' + plan + site-izni görür | P4✓(yeni alanlar opsiyonel/geri-uyumlu; fallback eski davranış)·P5✓(mevcut Serper/Tavily; yeni egress yok)·P8✓(İşlevsel+Güvenilirlik 3-kademe; 194 test)·P9✓(v4-pro tool-desteği canlı doğrulama beklemede—Groq kesin; maliyet/gecikme dürüstçe) | ADR-129 | TA-2 · canlı |
| C-032 | **Asistan kapasite/örnek/liste cevabı (junk-tekrar düzeltmesi)** — `ASSISTANT_CORE_RULES` (prompts.ts, tek kaynak → tek-atış + ajan): yeni "CAPABILITY/EXAMPLE/LIST REQUESTS" bloğu ("ne yapabilirsin / örnek / N madde" → çeşitli somut örnek, ready=false, tekrar yok); STYLE açık liste isteğinde temiz numaralı liste serbest (markdown sembolü yok). Heuristic `looksLikeCapabilityRequest` → 8-örnekli canned yanıt (latent junk-niyet bug'ı kapanır) | Düzeltici | Asistan "neler yapabilirsin"i gerçekten yanıtlar, aynı şablonu tekrarlamaz; kullanıcı yardımcı örnek listesi görür | P4✓(geri-uyumlu; RULE #1 + junk reddi korundu)·P5✓(yeni egress yok)·P8✓(İşlevsel+Kullanılabilirlik; 199 test)·P9✓(prompt davranışı canlı deploy sonrası gözlenir — dürüstçe) | ADR-130 | TA-2 · canlı |
| C-033 | **Mobil-web sihirbaz klavye/yerleşim düzeltmesi** — `new.tsx` intent ScrollView `contentContainerStyle` flexGrow:1+justify-end (mesajlar girişe yaslanır, boşluk yok); `deploy.yml` index.html injection'a `interactive-widget=resizes-content` viewport (klavye layout'u küçültür → footer üstte kalır). `+html.tsx` onurlanmadığı için CI injection kullanıldı; zoom korundu | Düzeltici | Klavye açılınca giriş/footer taşmaz, üstte boşluk kalmaz; sohbet düzenli görünür | P6✓(yalnız mobil-web sunum; native etkilenmez)·P8✓(Kullanılabilirlik; typecheck+biome+injection testi)·P9✓(klavye davranışı yerelde test edilemedi—CI-only web build; gerçek cihazda gözlenmeli — dürüstçe)·WCAG 1.4.4 (zoom korundu) | ADR-131 | TA-2 · canlı (CI deploy) |
| C-034 | **Sohbette gereksinim toplama (slot-filling)** — `ASSISTANT_CORE_RULES` "REQUIREMENTS GATHERING": konuya-özel slot tespiti + `collectedDetails` {label,value} her turda + eksik en önemli detayı tek-tek sor (~4'e kadar, "farketmez"de dur). QUESTION RULE #3 "en çok 2"→"yalnız izlemeyi değiştiren detaylar". Contract/domain/ReplySchema + 2 prompt şeması geri-uyumlu `collectedDetails`. Mobil `DetailsCard` (ready olmadan da) + i18n ×11 | Artımlı | Asistan ilgili detayları akıllıca toplar; kullanıcı neyin netleştiğini canlı görür ("devrim") | P4✓(geri-uyumlu; RULE #1 uydurma yasağı korundu; "farketmez" kayıtlı)·P8✓(İşlevsel+Kullanılabilirlik+29148; 200 test)·P9✓(soru 2→~4 ama anti-yorma korundu; prompt davranışı canlı gözlenir — dürüstçe) | ADR-132 | TA-2 · canlı |
| C-035 | **Stripe ödemeyi mobil uca bağlama** — backend Stripe akışı (checkout + webhook + gateway) zaten hazırdı; mobil "Yükselt" placeholder Alert'ti. `api.checkout` → `/v1/billing/checkout`, `subscription.tsx` checkout mutation → `WebBrowser.openBrowserAsync`, plan satırları dokunulabilir, `app/billing/success`+`cancel` dönüş sayfaları, i18n ×11 | Artımlı | Mobilden gerçek Stripe checkout başlatılır; ödeme uçtan-uca | P3✓(buy: Stripe)·P5✓(Pro yalnız imzalı webhook'la; istemci yazmaz)·P8✓(İşlevsel+Güvenlik; typecheck+biome; 200 test)·P9✓(Stripe panosu+Render env kullanıcıda; yerelde test edilemedi — dürüstçe; anahtarsız InMemory fallback Pro vermez)·27002(anahtar env secret) | ADR-133 | TA-2 · canlı (kod); anahtarlar bekliyor |
| C-036 | **Kişiye-özel zil bildirimi (hediye)** — migration 0018 `announcements.recipient_user_id` (nullable; NULL=global). gift-pro endpoint başarılı hediyede kişiye-özel `promo` duyurusu açar (best-effort); okuma rotası global+bu-kullanıcı filtreli (JS, migration-resilient), `recipientUserId` sızmaz. Mobil zil ekranı zaten şık gösterir; haber→zil zaten duyuruyla çalışıyor | Artımlı | Hediye edilince kullanıcı zilinde şık+basit bildirim görür; kişiye özel | P1✓(filtre backend; hedef sızmaz)·P5✓(yeni egress yok)·P8✓(İşlevsel+Güvenlik; 201 test)·P9✓(migration canlıya kullanıcı uygular—kod resilient; gift metni TR tek-dil — dürüstçe) | ADR-134 | TA-2 · canlı (kod); migration bekliyor |
| C-037 | **Duyuru çok-dillilik (templateKey + lang)** — migration 0019 `template_key`/`lang`. Sistem bildirimleri (hediye) templateKey ile istemci kullanıcı dilinde yerelleştirir (×11 `notif.giftPro*`); admin duyurusu `lang` alanıyla dile özel, kullanıcı kendi dili+global görür (?lang filtresi, JS resilient). Admin ekranına dil seçici; hediye TR fallback korunur | Artımlı | Hediye + duyurular her kullanıcının dilinde; admin dili panelden seçer | P4✓(eklemeli, geri-uyumlu; resilient)·P6✓(çok-dil sunum)·P8✓(İşlevsel+Taşınabilirlik; 202 test)·P9✓(migration kullanıcı uygular—kod resilient; sistem-şablon metni çeviride, panelden değil — dürüstçe) | ADR-135 | TA-2 · canlı (kod); migration bekliyor |
| C-038 | **Admin konsol nav reorganizasyonu** — admin/index.tsx 18-öğelik düz liste → 5 mantıksal grup (Analitik&büyüme · Kullanıcılar&abonelik · İçerik&iletişim · Yapay zekâ · Sistem&denetim), başlıklı. Kart/nabız/rozet korundu | Düzeltici | Admin paneli taranabilir/düzenli; kullanıcının gördüğü karışıklık giderildi | P6✓(yalnız sunum)·P8✓(Kullanılabilirlik+Bakımkolaylığı; typecheck+biome)·P9✓(backend 968-satır router split RİSKLİ → ayrı test-korumalı refactora bırakıldı, dürüstçe) | ADR-136 | TA-2 · canlı |
| C-039 | **admin.route.ts modülerleştirme (M1 / FAZ 1.1-1.4)** — 968-satır/41-rota monolit → 8 domain modülü (`admin/{system,ai,content,billing,users,channels,watches,support}.route.ts` + `_shared.ts`); her modül `register*(app,container[,audit])` ile AYNI app'e kaydeder (sub-app DEĞİL → OpenAPI yolları+davranış birebir). admin.route.ts 968→44 satır (saf kompozisyon + `audit` DRY helper). Hiçbir dosya >300 satır | Yeniden-yapılandırma (saf refactor) | Bakımkolaylığı temeli; her domain kendi route modülünde; M1.5 + diğer modüller için zemin; C-038'de ertelenen test-korumalı split tamamlandı | P4✓(OpenAPI diff = yalnız rota sırası; şema aynı)·P8✓(Bakımkolaylığı ana kazanım + İşlevsel korundu; **202 test 38/38**, taşınan rotalar runtime'da vurulur; 41 rota metot+yol benzersiz)·P9✓(FAZ 1.5 application/container split YAPILMADI—sıradaki; davranış birebir, migration yok — dürüstçe) | ADR-137 | TA-2 · canlı |
| C-040 | **container.ts modülerleştirme + ölü kod (M1 / FAZ 1.5)** — 487-satır composition root → `config/container/{types,builders,repositories}.ts` + 200-satır container.ts; db/in-memory **iki tekrarlı nesne literali → tek repo fabrikası** (`buildSupabase/InMemoryRepositories`); `Container` re-export (importer'lar dokunulmadı). Ölü kod: 3 dosya silindi (groq.assistant/deepseek.reasoner/groq.verifier — switchable deseni devre dışı bıraktı) + StaticAccessTokenProvider + okSchema export daraltıldı + bayat yorum düzeltildi. RAG stub'ı kasıtlı bırakıldı | Yeniden-yapılandırma (saf refactor) | "Yeni repo iki yere" bug sınıfı yok edildi; composition root analiz-edilebilir; M1 tamam | P4✓(Container sözleşmesi birebir; importer'lar değişmedi)·P8✓(Bakımkolaylığı/değiştirilebilirlik ana kazanım; typecheck merge'i Container'a karşı doğrular; **202 test 38/38**, in-memory fabrika runtime'da)·P9✓(Supabase repo dalı yalnız typecheck-doğrulandı—canlı DB testte yok; anahtarlar eski literalle birebir eşlendi; app.ts kapsam dışı — dürüstçe) | ADR-138 | TA-2 · canlı |
| C-041 | **Plan özellik-maddeleri + şık karşılaştırma kartı (M2 / FAZ 2.2)** — admin-yazılı, **dile-özel** özellik maddeleri (app_settings `plan.features`, migration YOK): `application/plan-features.ts` + `planFeaturesSchema`/`setPlanFeaturesInputSchema`; public `GET /v1/plans/features?lang=` + admin `GET/PUT /v1/admin/plan-features` (audit'li). Mobil: abonelik ekranı Free/Pro karşılaştırma kartları (lucide Check, accent Pro, i18n ×11 varsayılan ↔ API override) + `admin/plan-features.tsx` (dil seçici ×11 + madde editörü ekle/sil). 12 i18n anahtarı ×11 | Artımlı | Kullanıcının istediği "madde madde özellik + şık kart + admin-düzenlenebilir + çok-dilli" tam karşılandı | P4✓(yeni alanlar geri-uyumlu; boş→varsayılan)·P6✓(çok-dil sunum)·P8✓(İşlevsel+Kullanılabilirlik+Taşınabilirlik; **206 test 39/39** +dil izolasyonu; mobil typecheck/biome temiz)·P9✓(varsayılanlar i18n'de—admin override yazar, sıfırdan-her-dil zorunlu değil; app_settings kalıcılığı migration 0014'e bağlı—UI dürüst gösterir) | ADR-139 | TA-2 · canlı |
| C-042 | **Admin konsol boş-durum tutarlılığı (M6 / FAZ 6.1)** — paylaşılan `Empty` atomic bileşeni (`features/admin/ui.tsx`: lucide ikon dairesi + başlık + ipucu); 6 liste ekranı düz-metin boş-durumdan `Empty`'ye geçti (subs/watches/users/support/audit/announcements), 4-durum kuralının "boş" ayağı birleşti. Audit'e eksik 3 eylem-etiketi (plan.features/channels.config/email.prompt) | Düzeltici (tutarlılık) | Konsol liste boş-durumları tek-tip + tanınabilir; DRY tek kaynak | P8✓(Kullanılabilirlik+Bakımkolaylığı; mobil typecheck/biome temiz; davranış değişmez)·P9✓(yalnız liste boş-durumları; pano grafik-içi + destek dizgesi boş hâli dokunulmadı — dürüstçe) | ADR-140 | TA-2 · canlı |
| C-043 | **Log PII redaksiyonu (M9 / FAZ 9.1)** — `logger.emit()` SON kapısına `redactPii()`: duyarlı anahtar→`[redacted]`, string-içi e-posta/`Bearer`/JWT/sağlayıcı-jeton→maske (≤4 derinlik özyineli). Mevcut altyapı zaten güçlüydü (request-id korelasyon + yapısal log + PII-temiz çağrı yerleri); bu, yakalanmayan-hata `message`/`stack` artık riskini kapatan derinlemesine-savunma | Düzeltici (güvenlik sıkılaştırma) | P1 "0 PII egress" yedek kapısı; gelecekteki log çağrıları da güvenli | P1✓(log PII sızıntısı kapatıldı)·P5✓(derinlemesine-savunma; çağrı-yeri disiplini + bu kapı)·P8✓(Güvenlik+25012; **210 test 40/40**, opaque UUID yanlış-pozitif yok)·P9✓(regex sezgisel—asıl savunma çağrı-yeri; LLM-hata/kuyruk metrikleri M9.2'ye ertelendi — dürüstçe) | ADR-141 | TA-2 · canlı |
| C-044 | **Operasyon teslimat-sağlığı (M9 / FAZ 9.2 — kısmi)** — `getOps.deliveries`'e `successRate` (0-100; terminal=sent+delivered+failed, pending hariç) + `failed`; saf `deliveryHealth` util (test edilebilir). Mobil Operasyon: başarı oranı baş göstergesi (renkli çubuk + tone eşikleri, a11y progressbar). Contract+domain+mobil tip üçü hizalandı | Artımlı | Teslimat sağlığı tek bakışta görünür (eyleme dönük); dayanıklı DB verisinden | P8✓(Kullanılabilirlik+Güvenilirlik; **215 test 41/41** +pending-terminal-değil; typecheck/biome temiz)·P9✓(yalnız dayanıklı-türetilebilir dilim; LLM-hata-oranı=pencereli sayaç + kuyruk-derinliği=pg-boss v12 belirsiz → ayrı tura ertelendi, dürüstçe) | ADR-142 | TA-2 · canlı |
| C-045 | **RAG bilgi tabanı — pgvector deposu (M3 / FAZ 3.1)** — migration 0020 (vector ext + `embeddings` vector(768) + HNSW cosine + `match_embeddings` RPC + RLS service-role); domain `RagStore` (upsert/query/available) + `SupabaseRagStore`/`NullRagStore` + `indexDocuments` embed-yazım yolu + container `rag` + database.types. Boyut 768 (sağlayıcı-uyumlu) | Artımlı `[M]` | RAG depolama temeli; M3.2 korpus + M3.3 retrieval için zemin | P5✓(RLS kapalı-varsayılan, service-role)·P8✓(port/adapter; **220 test 42/42**; typecheck/biome)·P9✓(migration İZİNLE CANLIYA UYGULANDI + uçtan-uca doğrulandı [768-vektör insert→match_embeddings skor 1.0→temizlik]; korpus/retrieval akışı henüz yok — dürüstçe) | ADR-143 | TA-2 · **canlı (migration uygulandı)** |
| C-046 | **RAG korpus indeksleme — tespit batch worker (M3 / FAZ 3.2)** — `indexNewDetectionEvents` (watermark'lı app_settings; yeni tespitleri göm+upsert) + `listDetectionEventsSince` repo sorgusu (supabase+in-memory) + start-workers 5dk RAG tick (tek combined process). Dormant/no-key graceful (EmbeddingConfigError→watermark sabit) | Artımlı | embeddings korpusu CANLI dolmaya başlar (prod OPENAI ile); M3.3 retrieval'a hazır | P2✓(watermark→tekrar-gömme yok, maliyet sınırlı)·P8✓(İşlevsel+Perf; **224 test 43/43**; typecheck/biome)·P9✓(yalnız detection_events; check_runs/site-policy + retrieval henüz yok — dürüstçe) | ADR-144 | TA-2 · canlı |
| C-047 | **Gömme açık 'Pasif' durumu (ADR-145)** — `EmbeddingRouter`'a `setActive('none')`→active=null + kayıtlı 'none' varsayılana düşmez (anahtar korunur); mobil embeddings AKTİF/PASİF rozeti + 'Pasif (kapalı)' seçeneği. Canlı ayar 'none'→panel PASİF + worker 429 gürültüsü durur | Düzeltici (dürüstlük/UX) | OpenAI kotasız anahtar 'aktif' görünüyordu (429 spam); artık dürüst pasif + admin-kontrollü | P9✓(yanıltıcı 'aktif' giderildi; runtime sağlık-kontrolü yok—dürüstçe)·P8✓(Kullanılabilirlik+Doğruluk; **227 test 44/44**)·27001✓(anahtar silinmez) | ADR-145 | TA-2 · canlı |
| C-048 | **Kanal-bazlı teslimat sağlığı (M7 / FAZ 7.5 — kısmi)** — `channelHealth` util (deliveryHealth üzerine) + `getOps.deliveries.channelHealth` (contract+domain+supabase) + mobil 'kanal sağlığı' bölümü (kanal × başarı oranı, renkli tone). 'Hangi kanal bozuk' görünürlüğü | Artımlı | Operatör arızalı kanalı saptar (push OK / telegram fail); reliability görünürlüğü | P8✓(Güvenilirlik+Kullanılabilirlik; **229 test**; typecheck/biome)·P9✓(yalnız görünürlük; otomatik yeniden-deneme=sayaç migration+dedup riski → ayrı tur, dürüstçe) | ADR-146 | TA-2 · canlı |
| C-049 | **İlk-kullanım onboarding (M8 / FAZ 8.1)** — `useOnboarded` (AsyncStorage bayrağı) + 3-adımlı overlay (`onboarding.tsx`); son adım → /new (aktivasyon). Ana ekran üzerine overlay (root nav değişmedi). 4 tasarım skill + a11y (progressbar/role/label) + i18n ×11 (10 anahtar) | Artımlı | Yeni kullanıcı değeri anlar + ilk izlemeye yönlendirilir (aktivasyon) | P6✓(yalnız sunum; düşük risk—root nav korundu)·P8✓(Kullanılabilirlik+9241+WCAG; mobil typecheck/biome; i18n 11/11)·P9✓(giriş-sonrası gösterim + bildirim-izni eklenmedi—dürüstçe; mobil UI birim testi yok) | ADR-147 | TA-2 · canlı |
| C-050 | **Watcher limiti yükselt-bildirimi (M5 / FAZ 5.4 — kısmi)** — free kullanıcı limite takılınca bell'de tek-seferlik 'Pro'ya yüksel' sistem bildirimi (`templateKey=watchLimit`, recipient, dedup `existsForRecipient`); mobil localize + i18n ×11. ADR-134/135 altyapısı; gift dışındaki ilk tetikleyicili şablon | Artımlı | Limit→yükseltme dönüşüm nudge'ı; spam yok (ömürde bir) | P8✓(İşlevsel+Kullanılabilirlik; **231 test** +dedup; typecheck/biome)·P9✓(yalnız watchLimit şablonu; diğer şablonlar+CTA yok; 3/100 i18n'de sabit — dürüstçe)·27001✓(recipient sızmaz) | ADR-148 | TA-2 · canlı |
| C-051 | **Admin global arama (M6 / FAZ 6.3 — kısmi)** — `searchAdmin` (mevcut list metotlarını süzer, yeni repo yüzeyi yok) + `GET /v1/admin/search?q=` + `adminSearchSchema`; mobil `admin/search.tsx` (debounce, kategorili sonuç, kullanıcı/abonelik→detay) + konsol 'Ara' kartı | Artımlı | Tek kutudan kullanıcı/watcher/abonelik bul (destek/ops) | P8✓(Kullanılabilirlik+9241+WCAG; **235 test** +4; typecheck/biome)·P9✓(ölçek: liste-süzme küçük veri için—büyürse DB ILIKE; hızlı-eylemler/kısayol yok; watcher satırı tappable değil — dürüstçe) | ADR-149 | TA-2 · canlı |
| C-065 | **Sihirbaz 6-adım + Alarm Stüdyosu + fizibilite notu sadeleştirme (ADR-163)** — `new.tsx` FSM dinamik 5↔6 adım (Alarm açıksa "Alarm sesi" stüdyo adımı girer); ayrı "Kanallar" sayfası (ADR-162 5-toggle); ses seçici kendi kaliteli adımına taşındı; `FeasibilityCard` ham LLM robots.txt notu yerine temiz `siteOk/siteLimited`. i18n ×11 (steps/titles.channels+alarm, alarmStudio*, site*) | Düzeltici+Artımlı | Kullanıcı 6. sayfada kanal seçer + alarm açınca stüdyo açılır; jargon temizlendi; Telegram/e-posta artık appConfig'e bağlı (Render'da token/key MEVCUT → aktif) | P8✓(Kullanılabilirlik+Doğruluk; mobil/backend typecheck; biome apps/mobile temiz; 265 backend test değişmedi; 11/11 i18n)·25012✓(gösterilen veri sadeliği)·WCAG✓(dinamik progressbar/switch)·P9✓(ek kanal hâlâ HESAP düzeyi—per-watcher migration ister; e-posta teslimi RESEND_FROM doğrulamasına bağlı; eski-önbellek için sert yenileme; cihazda test edilmedi — dürüstçe) | ADR-163 | TA-2 · deploy bekliyor |
| C-064 | **Tam ekran çalan-alarm ekranı + watcher'da çoklu-kanal seçimi (ADR-162)** — `app/(app)/alarm.tsx` (ses döngü/expo-audio + titreşim + pulse/reduce-motion + Durdur/Ertele); `notifications.ts` ön planda alarm→ekran + `registerResponseListener` (bildirime dokunma). Sihirbaz "alert" adımı → 5 kanal toggle + "Hepsini seç" (Bildirim/Alarm cihaz-yerel watch-başına; Telegram/E-posta/WhatsApp hesap düzeyi `setChannels`, appConfig'le "yakında"). i18n ×11 (`alarm` bloğu + wizard anahtarları) | Artımlı | Alarm artık tam ekran çalar + kullanıcı her watcher'da haber-verme yollarını seçer (hepsini-seç) | P8✓(Kullanılabilirlik+İşlevsel+Taşınabilirlik; mobil/backend typecheck; **biome 104 dosya temiz**; 265 backend test değişmedi; 11/11 i18n)·WCAG✓(switch rol+label/≥44pt/reduce-motion)·P9✓(**ek kanal HESAP düzeyi, per-watcher relay migration ister — açık-izinli sonraki iş**; alarm öncelikle ön plan/dokunma, tam-kapalı arka plan native handler ister; cihazda test edilmedi — dürüstçe) | ADR-162 | TA-2 · deploy bekliyor |
| C-063 | **Tanıtım sitesi 11-dil genişlemesi + N-dilli SSG motoru (ADR-161)** — ikili (TR↔EN) SLUG_MAP kaldırıldı → `LOCALES`(11) tek kaynak + eşlenik eşleştirme sayfa-key'inden; N-yönlü hreflang + `x-default→EN` + 11-dilli dil değiştirici; RTL (`ar` → `dir="rtl"`); llms.txt/sitemap/JSON-LD N-dile genişledi; 9 yeni `content.*.mjs` (de/es/fr/pt/ru/ar/hi/ja/zh). 36→**198 sayfa**. Dürüstlük kısıtı: yalnız kamuya-açık senaryolar, login/captcha/garanti/uydurma-istatistik/aciliyet YOK | Artımlı | Site uygulamayla aynı 11 dilde global; SEO/GEO N-dile genişledi; ürün davranışı değişmez | P8✓(Taşınabilirlik+Kullanılabilirlik+Doğruluk; **198 sayfa**; `node --test`=**21/21**; biome temiz 22 dosya; 176/176 metaDescription [50,165] + dürüst — kendim doğruladım)·25012✓(içerik veri kalitesi)·29148✓("11 dil"+"yapamayacağını iddia etme" izlenebilir)·P9✓(**9 dil çeviri ilk-geçiş/LLM — anadil redaksiyonu yok**; yapı+dürüstlük+uzunluk makinece doğrulandı, çeviri kalitesi insan-onaylı değil — dürüstçe) | ADR-161 | TA-2 · deploy bekliyor |
| C-062 | **App ikonu + admin-yapılandırılır plan limitleri (ADR-160)** — kullanıcı logosu (1024²) → app.json icon + Android adaptiveIcon. `plan-config.ts` (app_settings override, clamp'li, defaults-fallback) + use-case'ler settings-verilince okur (geri-uyumlu) + `GET/PUT /v1/admin/plan-entitlements` (audit) + mobil "Plan Limitleri" ekranı. Fiyat(Play)+metin(ADR-139)+limit(ADR-160) artık kodsuz değişir | Artımlı | Admin watcher limiti/sıklık/alarm/sesleri redeploysuz değiştirir; Play ikonu hazır | P8✓(Kullanılabilirlik+config-as-data; **265 test** +4; typecheck/biome; WCAG)·27001✓(admin-guard+audit)·P9✓(adaptive tam-bleed maske kırpması; mevcut watch'lar sonraki reconcile'da uyarlanır — dürüstçe) | ADR-160 | TA-2 · canlı |
| C-061 | **RevenueCat (Android IAP) webhook → Pro yetkisi — backend altyapısı (ADR-159)** — açık `POST /webhooks/revenuecat` (Authorization env-giziyle doğrulanır; dormant-güvenli) + `revenueCatToPaymentEvent` → mevcut `applyPaymentEvent` (Stripe ile ortak boru). Render'a güçlü rastgele `REVENUECAT_WEBHOOK_AUTH`. Play Billing yolu (Google=MoR, şirketsiz/bütçesiz) | Artımlı | Android IAP sunucu-yetki altyapısı hazır; kullanıcı RC anahtarını bağlayınca aktif | P8✓(İşlevsel+Güvenlik+DRY; **261 test** +7; typecheck/biome)·27001✓(webhook auth + güçlü giz)·P9✓(**app native SDK + RC config + cihaz testi HENÜZ yok** — Google doğrulaması beklemede + native test gerek; interval heuristik — dürüstçe) | ADR-159 | TA-2 · canlı |
| C-060 | **Android/EAS build hazırlığı — eas.json placeholder + yanlış URL düzeltildi (ADR-158)** — 3 profilde gerçek backend URL (`watcher-backend-p0hi`; eski `watcher-backend`=çözülmüyor/http=000) + gerçek Supabase URL + public anon key. Play Billing yoluna hazırlık | Düzeltici | `eas build -p android` artık çalışan uygulama üretir (ölü backend/sahte Supabase giderildi) | P8✓(İşlevsel Doğruluk; biome; 0 placeholder/3 URL canlı-doğrulandı)·27001✓(yalnız public anon key; secret değil)·P9✓(app ikonu 1024² hâlâ gerekli — tasarım varlığı; dürüstçe) | ADR-158 | TA-2 · canlı |
| C-059 | **Zorunlu sözleşme kapısı + ileri/geri oklu onboarding (ADR-157)** — `TermsGate` (layout düzeyinde; kabul edilene dek Tabs render EDİLMEZ; Koşullar+Gizlilik linkleri + zorunlu onay kutusu + disabled-until-checked) + `useTermsAccepted` (AsyncStorage `terms.v1`); onboarding'e ChevronLeft/Right ileri-geri navigasyon. i18n ×11 (`terms` + `onboarding.back`) | Artımlı | Giriş→zorunlu sözleşme→kabul→rehber(ileri/geri)→uygulama; yasal onay + 4 tasarım skill + a11y | P8✓(Kullanılabilirlik+Uyumluluk+9241+WCAG; mobil typecheck/biome; i18n 11/11)·P9✓(onay cihazda—sunucu kaydı yok, ileride migration; rehber tasarım-sistemi, literal screenshot değil — dürüstçe) | ADR-157 | TA-2 · canlı |
| C-058 | **Kritik güvenlik denetimi + düzeltmeler: SSRF + ödeme/auth fail-fast (ADR-156)** — 9-alan derin denetim. **C1 SSRF** (DNS-çözümlü private-IP blok + manual-redirect re-validate; yeni `ssrf-guard.ts`, origin+site-policy taşındı — bulut-metadata/iç-ağ exfil kapandı) · **C2** Stripe boş-webhook-secret fail-open → fail-fast · **H1** prod'da DevAuth/InMemory fail-open → reddet. Sağlam: Telegram-secret/admin-guard/JWT/delete-scope/redaksiyon | Düzeltici (güvenlik) | Metadata/iç-veri sızıntısı + bedava-Pro + auth-bypass landmine'ları kapatıldı | P5✓(SSRF+secret+fail-fast; **254 test** +4; typecheck/biome)·P1✓(iç-veri/PII egress kapandı)·P9✓(M1-IDOR/H2-ratelimit/M2-CORS gerekçeyle ertelendi; IP-pinning yok→dar TOCTOU dürüstçe açık) | ADR-156 | TA-2 · canlı |
| C-057 | **Çapraz-platform onay — `Alert.alert` web'de kırık tuşlar düzeltildi (ADR-155)** — yeni `confirmAsync` (web: window.confirm, native: Alert.alert); 5 onay yeri taşındı (hesap sil / watcher sil ×2 / kullanıcı sil / broadcast) + subscription hata→toast. Explore tam-denetim: tek kök-neden, rota/api/handler temiz | Düzeltici | Mobil-web'de kritik tuşlar (sil/gönder) artık çalışır; native korundu | P8✓(İşlevsel Doğruluk+Taşınabilirlik; mobil typecheck/biome; backend 250 test)·P9✓(window.confirm sade ama çalışır; özel modal ileride; canlı görsel-test deploy sonrası — dürüstçe) | ADR-155 | TA-2 · canlı |
| C-056 | **Ek kanallar sistem-sağlığında + e-posta (Resend) aktive (ADR-154)** — `buildServiceHealth`'e E-posta/Telegram/WhatsApp (gerçek-config sağlığı). Resend env (key + from=onboarding@resend.dev) Render'a girildi; canlı test e-postası gönderildi (id'li) | Artımlı | Admin tüm kanal durumunu görür; e-posta kanalı çalışır (test modu) | P8✓(Kullanılabilirlik; **250 test**; typecheck/biome)·P9✓(onboarding@resend.dev → yalnız hesap adresine gider; gerçek kullanıcı için domain doğrulama gerek — dürüstçe; serviceHealth=config var, runtime gönderim değil) | ADR-154 | TA-2 · canlı |
| C-055 | **Telegram botu konuşur + tek-dokunuş otomatik bağlanır (ADR-153)** — `TelegramBotApi` (getMe/sendText/setWebhook; secret token'dan türetilir) + açık `POST /telegram/webhook` (secret_token doğrulamalı) + `handleTelegramUpdate` (/start kod→hesaba bağla, kodsuz→karşılama, rastgele metin→"bildirim botuyum" açıklaması; tr/en) + `InMemoryTelegramLinkStore` (tek-kullanımlık, 10dk) + `POST /v1/me/channels/telegram-link` (derin bağlantı) + mobil "Telegram'ı bağla" düğmesi + boot setWebhook. E-posta/WhatsApp altyapısı zaten hazır (doğrulandı) | Artımlı | Bot artık yanıt verir; kullanıcı chat kimliğiyle uğraşmadan tek dokunuşla bağlanır | P8✓(Kullanılabilirlik+İşlevsel; **250 test** +8; typecheck/biome; i18n 11/11)·27001✓(webhook secret_token; link kodu tek-kullanımlık/TTL)·P9✓(bellek-içi store=tek-instance varsayar→çok-instance'ta DB'ye; e-posta/WhatsApp hâlâ kimlik bilgisi ister — dürüstçe) | ADR-153 | TA-2 · canlı |
| C-054 | **Kanal kullanılabilirliği = admin AND sunucu-kimlik-bilgisi (sessiz başarısızlık giderildi — ADR-152)** — `configuredAvailability` (buildChannels çıktısından) + `effectiveAvailability` (AND); `/v1/config` `channels`=etkin + yeni `channelsConfigured`; admin `channel-config` `{availability,configured}`. Kanal ekranı "kapalı" vs "henüz hazır değil" ayrımı (i18n ×11 `channels.notReady`); admin ekranı "kimlik bilgisi yok" uyarısı (lucide Lock) | Düzeltici/Artımlı | Açık toggle artık gerçeği yansıtır; founder eksik anahtarı görür; sessiz başarısızlık biter | P8✓(Kullanılabilirlik+İşlevsel Doğruluk; **242 test** +3; mobil typecheck/biome; i18n 11/11)·P9✓(**kanalı tek başına çalıştırmaz**: kimlik bilgisi prod'a eklenmeli; configured=env-var varlığı, anahtar geçerliliği değil — runtime sağlık ayrı; dürüstçe) | ADR-152 | TA-2 · canlı |
| C-053 | **Push-token'sız shared teslimde ek-kanal atlanması düzeltildi (ADR-150 incelemesinin devamı)** — `dispatchEventDeliveries` token=0'da `continue` ile ek-kanal bloğunu atlıyordu → erken-çıkış yalnız personal'e daraltıldı (cihaz-gate korundu); shared'da token yoksa kanal (email/telegram/whatsapp) yine denenir. Canlı: 23/23 failed, hepsi shared/tek-alıcı/0-push-token/enabled=[whatsapp,email] | Düzeltici | Push cihazı olmayan ama kanal tanımlı shared alıcı artık ulaşılabilir (kanal kimliği eklenince) | P8✓(İşlevsel Doğruluk+Güvenilirlik; **239 test** +3; typecheck/biome)·P9✓(**canlı 23 başarısızlığı TEK BAŞINA çözmez**: alıcıda push-cihazı yok + prod'da kanal-kimliği yok→buildChannels=[]; bugün davranış değişmez/ileriye-doğru; olaylar feed'de görünür — dürüstçe) | ADR-151 | TA-2 · canlı |
| C-052 | **Kod-incelemesi düzeltmeleri (ADR-137→149 öz-denetim)** — /code-review high-effort. KRİTİK: getOps teslimat-sağlığı `sent_at` filtresiyle TÜM başarısızları dışlıyordu (failed→sent_at NULL) → hep %100; canlıda 23/23 failed gizleniyordu → event-penceresinden türet (düzeltildi+canlı doğrulandı). + log TOKEN_RE {40,} aşırı-redaksiyon kaldırıldı + ops eşik DRY. Refactor denetimi (route/container/arayüzler) TEMİZ | Düzeltici | Yanıltıcı sağlık metriği + log bozulması giderildi; refactor güvenli doğrulandı | P8✓(İşlevsel Doğruluk+Güvenilirlik gözlemi; **236 test**; getOps canlı-DB doğrulandı)·P9✓(düşük-şiddet bulgular notlandı/ertelendi; byChannel ölü-alan + watermark-kenarı dürüstçe açık) | ADR-150 | TA-2 · canlı |
