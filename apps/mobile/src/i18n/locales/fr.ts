import type { Catalog } from "./tr";

export const fr: Catalog = {
  onboarding: {
    skip: "Passer",
    next: "Suivant",
    createFirst: "Crée ta première surveillance",
    later: "Plus tard",
    s1Title: "Bienvenue sur Whenly",
    s1Body:
      "Whenly surveille le monde réel pour toi — baisses de prix, créneaux de rendez-vous, annonces de résultats et plus encore.",
    s2Title: "Dis-le simplement avec tes mots",
    s2Body: "Décris ce que tu veux suivre ; l'IA trouve la source et te tient informé.",
    s3Title: "Ne rate plus rien",
    s3Body:
      "Reçois une notification instantanée dès qu'un changement important survient. Crée ta première surveillance maintenant.",
    back: "Retour",
  },
  terms: {
    gateTitle: "Contrat d'utilisation",
    gateIntro:
      "Avant d'utiliser Whenly, vous devez accepter les Conditions d'utilisation et la Politique de confidentialité.",
    readAndAccept:
      "J'ai lu et j'accepte les Conditions d'utilisation et la Politique de confidentialité.",
    acceptButton: "Accepter et continuer",
  },
  tabs: {
    feed: "Alimentation",
    watchers: "Watchers",
    subscription: "Abonnement",
    settings: "Paramètres",
    admin: "Administrateur",
  },
  common: {
    cancel: "Annuler",
    delete: "Supprimer",
    save: "Enregistrer",
    soon: "bientôt",
    loadError: "échec de chargement",
    continue: "Continuer",
    back: "Retour",
    create: "Créer",
    send: "Envoyer",
    close: "Fermer",
    active: "Actif",
    paused: "Suspendu",
    open: "ouvert",
    closed: "fermé",
    new: "Nouveau",
    justNow: "juste maintenant",
    minAgo: "{{n}}m il y a",
    hourAgo: "{{n}}h il y a",
    dayAgo: "{{n}}j il y a",
    openMap: "ouvrir la carte",
    openMapA11y: "Ouvrir le lieu sur la carte : {{coords}}",
  },
  login: {
    tagline: "veilleur intelligent",
    hero: "Tout ce que vous voulez\nregarder, instantanément.",
    heroSub: "L'IA suit votre sujet en continu et vous alerte le moment venu.",
    title: "Se connecter",
    subtitle: "Connectez-vous à votre compte et reprenez où vous en étiez.",
    devSubtitle: "Mode de développement · identifiant de l'utilisateur",
    email: "adresse email",
    emailPlaceholder: "nom@exemple.fr",
    password: "mot de passe",
    showPass: "Afficher le mot de passe",
    hidePass: "Cacher le mot de passe",
    signIn: "Se connecter",
    or: "ou",
    google: "Continuer avec Google",
    googleOff: "La connexion Google n'est pas encore activée — utilisez l'e-mail pour l'instant.",
    devId: "identifiant de l'utilisateur de développement",
    devNote: "Développement · jeton = identifiant de l'utilisateur",
    terms: "En continuant, vous acceptez les termes et la politique de confidentialité de Whenly.",
  },
  feed: {
    why: "Détecté avec {{n}}% de confiance — touche pour les détails",
    activity: "Activité",
    last7days: "7 derniers jours",
    activityA11y: "{{total}} détections au cours des 7 derniers jours",
    title: "Alimentation",
    subtitle: "Mises à jour en temps réel de tous vos observateurs.",
    statWatchers: "Total observateurs",
    statToday: "Détectations aujourd'hui",
    statScans: "Scans 24h",
    statNotifs: "Notifications",
    filterAll: "Tout",
    filterDetections: "Détectations",
    filterWarnings: "Avertissements",
    filterUnread: "Non lus",
    newCount: "{{n}} nouvelles détectations",
    markAll: "marquer tout lu",
    markAllA11y: "Marquer toutes les détectations comme lues",
    emptyTitle: "Aucune détectation pour l'instant",
    emptyHint: "Lorsque vos observateurs détectent quelque chose, cela apparaît ici.",
    emptyFilterTitle: "Aucun résultat pour ce filtre",
    emptyFilterHint: "Essayez un filtre différent.",
    fallback: "Quelque chose a été détecté.",
    wasCorrect: "Était-ce correct?",
    thanks: "Merci!",
    noted: "Noté — nous allons nous améliorer.",
    correct: "Correct",
    wrong: "Incorrect",
    groupCount: "{{n}} détectations",
    detection: "Détectation",
    warning: "Avertissement",
    newBadge: "nouveau",
    newBadgeN: "{{n}} nouveau",
  },
  watchers: {
    filterAll: "Tout",
    filterActive: "Actif",
    filterPaused: "Suspendu",
    research: "recherche",
    menuA11y: "Menu du Watcher",
    menuOpen: "Ouvrir la recherche",
    pause: "Mettre en pause",
    resume: "Reprendre",
    completed: "Résultat trouvé",
    shared: "Partagé",
    personal: "Personnel",
    alert: "Alerte",
    perDay: "une fois par jour",
    perHours: "tous les {{n}}h",
    perMin: "tous les {{n}}min",
    footer: "{{n}} watchers actifs · tout suivre pour vous",
    emptyTitle: "Aucun Watcher pour l'instant",
    emptyHint: "Créez votre premier Watcher avec le bouton plus en bas.",
    deleteTitle: "Supprimer le Watcher",
    deleteMsg: 'Supprimer définitivement "{{intent}}"?',
    fail: "Une erreur est survenue",
    newFab: "Créer un nouveau Watcher",
  },
  wizard: {
    steps: {
      intent: "Qu'est-ce à regarder",
      source: "Source",
      freq: "Fréquence",
      alert: "Alertes",
      channels: "Canaux",
      alarm: "Son d'alarme",
      review: "Aperçu",
    },
    titles: {
      intent: "Qu'est-ce que nous devrions regarder?",
      source: "Préférence de la source",
      freq: "Combien de fois devrions-nous vérifier?",
      alert: "Comment devrions-nous vous alerter?",
      channels: "Comment devrions-nous vous alerter?",
      alarm: "Son d'alarme",
      review: "Summary & confirm",
    },
    greeting:
      "Bonjour ! Qu'est-ce que je devrais vous notifié à propos de ? Décrivez-le naturellement — si c'est trop large, je poserai une question pour le resserrer.",
    suggestTitle: "Ou commencez avec un exemple:",
    suggestPersonal: "Personnel",
    suggestBusiness: "Entreprise",
    ready: "prêt à regarder",
    readyHint: 'Fréquence suggérée : {{freq}} · appuyez sur "Continuer" ou continuez à taper.',
    planTitle: "Plan de recherche",
    collectedTitle: "Détails recueillis",
    feasibility: {
      can: "suivable",
      partial: "partiellement suivable",
      cannot: "non suivable directement",
    },
    feasibilityHint: {
      can: "C'est clairement suivable sur le web public ; je surveille la source officielle et les actualités et je vous alerte en cas de changement.",
      partial:
        "Seul un signal public indirect est suivable (p. ex. un portail de connexion/réservation) — je surveille l'annonce publique associée, pas le portail lui-même.",
      cannot:
        "Il n'y a pas de signal web public directement suivable pour cela (entièrement derrière une connexion/adhésion) ; néanmoins, si une annonce publique associée paraît, je peux la surveiller.",
    },
    feasibilityPlan: "Comment je vais surveiller",
    chatPlaceholder: "Typez un message…",
    chatA11y: "Envoyez un message à l'assistant",
    sendA11y: "Envoyer",
    chatError: "Quelque chose s'est mal passé — essayez à nouveau ?",
    you: "Vous",
    assistantRole: "Assistant",
    useMyText: "utiliser ce que j'ai écrit comme intention",
    assistError:
      "Impossible de joindre l'assistant — réessayez ou utilisez directement votre texte.",
    intentAccepted: 'Compris, je surveille : "{{intent}}". Continuez avec "Suivant".',
    needIntent: "Clarifiez le sujet avec l'assistant en premier.",
    sourceHint:
      "Quelle source devrait l'engin prioriser ? Auto : page officielle en direct → recherche officielle → dernières 24h de nouvelles.",
    sonar: "Sonar — analyse approfondie",
    stopAfterHit: "Arrêter une fois trouvé",
    stopAfterHitHint:
      "Après le premier vrai résultat, cette veille se met en pause ; reprenez-la quand vous voulez.",
    sonarHint:
      "Vérification en plusieurs passes et plus de sources à chaque contrôle. Plus complet, un peu plus lent.",
    srcAuto: "Auto",
    srcAutoD: "Recommandé",
    srcOfficial: "Officiel",
    srcOfficialD: "Site institutionnel en premier",
    srcNews: "Nouvelles",
    srcNewsD: "Premières 24h en premier",
    srcWeb: "Web",
    srcWebD: "Recherche générale",
    srcSocial: "Médias sociaux",
    srcShop: "Prix/Produit",
    freqHint:
      "Des vérifications plus fréquentes signifient des alertes plus rapides (plan gratuit : 60 min minimum).",
    freqNames: {
      f1: "1 min",
      f15: "15 min",
      f60: "1 heure",
      f360: "6 heures",
      f720: "12 heures",
      f1440: "Journalier",
    },
    freqDescs: {
      f1: "Instantané",
      f15: "Très rapide",
      f60: "Équilibré",
      f360: "Faible",
      f720: "Très faible",
      f1440: "Le plus faible",
    },
    alertSilent: "Silencieux",
    alertNotify: "Notification",
    alertAlarm: "Alarme",
    alarmPro: "L'alarme et les sons personnalisés sont des fonctionnalités Pro.",
    allSoundsPro: "Tous les sons sont une fonctionnalité Pro.",
    soundPlay: "Lire {{name}}",
    soundStop: "Arrêter",
    soundCustomPick: "Choisir un son depuis l'appareil",
    soundNote:
      "Touche ▶ pour écouter. En Pro, tu peux choisir ton propre son depuis l'appareil. Le son de notification est inclus dans le build Android.",
    rIntent: "qu'est-ce à regarder",
    rSource: "source",
    rFreq: "fréquence",
    rAlert: "alerte",
    rNote: "Le plan gratuit permet 3 watchers et une fréquence minimale de 60 min.",
    rChannels: "canaux",
    channelsHint:
      "Lorsque cette surveillance se déclenche, comment devons-nous vous joindre ? Vous pouvez en choisir plusieurs.",
    selectAll: "Tout sélectionner",
    chPush: "Notification",
    chPushD: "Recevez une notification push instantanée sur votre téléphone.",
    chAlarm: "Alarme",
    chAlarmD: "Votre téléphone sonne ; alerte plein écran avec son.",
    chTelegramD: "Recevez-le sous forme de message Telegram.",
    chEmailD: "Recevez la nouveauté par e-mail.",
    chWhatsappD: "Recevez-le sous forme de message WhatsApp.",
    relayBadge: "compte",
    relayNote:
      "Telegram, e-mail et WhatsApp sont liés à votre compte — en activer un s'applique à tous vos watchers partagés, et ils ne fonctionnent que pour les surveillances sans filtre personnel.",
    connectChannel: "Connecter",
    alarmStudioTitle: "Studio d'alarme",
    alarmStudioHint:
      "Choisissez et écoutez le son qui sera joué lorsque cette surveillance déclenche une alarme.",
    siteOk: "Cette source semble accessible à une surveillance automatique.",
    siteLimited:
      "La surveillance directe de cette source peut être limitée ; je surveillerai l'annonce publique associée.",
  },
  alarm: {
    title: "Alarme",
    defaultBody: "Le changement que vous surveilliez s'est produit.",
    stop: "Arrêter",
    snooze: "Répéter dans 5 minutes",
    a11yScreen: "Écran d'alarme — l'événement que vous surveilliez s'est produit",
  },
  sub: {
    currentPlan: "PLAN ACTUEL",
    compare: "Fonctionnalités du forfait",
    recommended: "Recommandé",
    featFreeWatches: "3 surveillances actives",
    featFreeFreq: "Vérifications toutes les heures",
    featFreeChannels: "Notifications push",
    featFreeAssistant: "Assistant de surveillance IA",
    featProWatches: "100 surveillances actives",
    featProFreq: "Vérifications jusqu'à chaque minute",
    featProAlarm: "Alertes d'alarme (sonores)",
    featProSounds: "100 sons de notification",
    featProChannels: "Telegram · WhatsApp · e-mail",
    featProSupport: "Assistance prioritaire",
    proName: "Plan professionnel",
    freeName: "Plan gratuit",
    minFreq: "min {{n}} min vérifications",
    alarmOn: "alarme activée",
    alarmPro: "alarme (pro)",
    soundsOn: "tous les sons",
    soundsPro: "sons (pro)",
    usage: "Utilisation de votre watcher",
    upgrade: "Mettre à jour le plan",
    upgradeMsg:
      "Les paiements arrivent très bientôt. Une fois opérationnel, vous passerez en Pro en un seul tapot ici.",
    billing: "détails de facturation",
    period: "Période",
    monthly: "Mensuel",
    yearly: "Annuel",
    amount: "Montant",
    status: "Statut",
    stActive: "Actif",
    stCancel: "Annulé",
    stPeriodEnd: "annule à la fin de la période",
    renewal: "Renouvelé",
    cancelBtn: "Annuler à la fin de la période",
    plans: "mettre à jour le plan",
    plansNote:
      "Les paiements arrivent bientôt — les prix sont indiqués ci-dessous, mais la mise en vente n'est pas encore activée.",
    perMonth: "mo",
    perYear: "yr",
    checkoutError: "Impossible de démarrer le paiement — réessayer ?",
    checkoutLoading: "Redirection…",
    invoices: "historique des factures",
    invoicesEmpty: "Aucune facture encore",
    invoicesHint:
      "Une fois les paiements opérationnels, votre historique des factures sera listé et téléchargeable ici.",
  },
  billing: {
    successTitle: "Paiement reçu",
    successBody:
      "Ton abonnement Pro est en cours de traitement ; il apparaîtra sur ton compte dans quelques secondes.",
    cancelTitle: "Paiement annulé",
    cancelBody: "Rien n'a changé. Tu peux réessayer quand tu veux.",
    toApp: "Retour à l'app",
  },
  notif: {
    giftProMonth: {
      title: "Ton abonnement Pro est prêt",
      body: "Un mois de Pro gratuit t'a été offert. Toutes les fonctions Pro sont débloquées — profite.",
    },
    giftProYear: {
      title: "Ton abonnement Pro est prêt",
      body: "Un an de Pro gratuit t'a été offert. Toutes les fonctions Pro sont débloquées — profite.",
    },
    watchLimit: {
      title: "Tu as atteint ta limite de surveillances",
      body: "Le forfait gratuit permet 3 surveillances actives. Passe à Pro pour en avoir jusqu'à 100 et des vérifications plus fréquentes.",
    },
  },
  suggest: {
    appointment: {
      label: "Rendez-vous",
      sentence:
        "Préviens-moi quand le rendez-vous officiel dont j'ai besoin (visa, consulat, etc.) s'ouvre",
    },
    doctor: {
      label: "Rendez-vous médical",
      sentence: "Préviens-moi quand le rendez-vous médical dont j'ai besoin s'ouvre",
    },
    medicine: {
      label: "Médicament / stock",
      sentence: "Préviens-moi quand le médicament ou l'article recherché est de nouveau disponible",
    },
    rental: {
      label: "Annonce immobilière",
      sentence:
        "Préviens-moi quand une nouvelle annonce de location/vente paraît dans ma zone et mon budget",
    },
    restock: {
      label: "Réassort",
      sentence: "Préviens-moi dès que le produit que je suis est de nouveau en stock",
    },
    tickets: {
      label: "Billets",
      sentence: "Préviens-moi quand les billets de l'événement que je suis sont mis en vente",
    },
    priceDrop: {
      label: "Baisse de prix",
      sentence: "Préviens-moi quand le prix d'un produit passe sous mon objectif",
    },
    result: {
      label: "Résultats",
      sentence: "Préviens-moi quand le résultat d'examen ou de candidature attendu est annoncé",
    },
    tender: {
      label: "Appel d'offres",
      sentence: "Préviens-moi quand un nouvel appel d'offres ou RFP est publié dans mon domaine",
    },
    regulation: {
      label: "Réglementation",
      sentence: "Préviens-moi quand une nouvelle réglementation concernant mon secteur paraît",
    },
    grant: {
      label: "Subvention",
      sentence: "Préviens-moi quand un nouvel appel à subvention ou aide s'ouvre",
    },
    competitor: {
      label: "Concurrent",
      sentence: "Préviens-moi quand le prix ou le site de mon concurrent change",
    },
    brand: {
      label: "Suivi de marque",
      sentence: "Préviens-moi quand ma marque est citée négativement en ligne",
    },
    expiry: {
      label: "Expiration",
      sentence: "Préviens-moi avant l'expiration de mon domaine, certificat ou abonnement",
    },
  },
  aiProfile: {
    title: "Personnalisation IA",
    intro:
      "Présente-toi à l'IA et indique ce à quoi prêter une attention particulière ; l'assistant l'utilise.",
    about: "À propos de toi",
    aboutHint: "ex. Je suis vendeur e-commerce et suis des produits tech.",
    attention: "Attention supplémentaire",
    attentionHint: "ex. Fais surtout attention aux changements de prix et au stock.",
    saved: "Enregistré",
  },
  appearance: {
    title: "Apparence",
    intro: "Personnalisez le thème, la couleur d'accent et le mouvement de l'app.",
    theme: "Thème",
    accent: "Couleur d'accent",
    accentHint: "Les boutons, en-têtes et accents prennent cette couleur.",
    motion: "Mouvement",
    motionFull: "Animation complète",
    motionReduced: "Réduire",
    motionHint: "Réduire les transitions et micro-animations (accessibilité).",
    preview: "Aperçu",
    previewBtn: "Bouton d'exemple",
    previewBadge: "Étiquette",
    accentIndigo: "Indigo",
    accentBlue: "Bleu",
    accentViolet: "Violet",
    accentFuchsia: "Fuchsia",
    accentRose: "Rose",
  },
  channels: {
    title: "Canaux de notification",
    sub: "Telegram · E-mail · WhatsApp",
    intro:
      "EN PLUS du push, nous pouvons aussi envoyer les nouveautés ici. Le push est toujours actif ; ceci est en supplément. Fonctionne uniquement pour les watchers partagés (sans filtre personnel).",
    telegram: "Telegram",
    telegramNote: "Envoie /start à notre bot et colle l'ID de chat ici. Gratuit et le plus rapide.",
    telegramPlaceholder: "ID de chat Telegram",
    email: "E-mail",
    emailNote: "Recevoir les nouveautés par e-mail.",
    emailPlaceholder: "toi@email.com",
    whatsapp: "WhatsApp",
    whatsappNote:
      "Saisis ton numéro au format international (+33…). Note : le WhatsApp automatique nécessite une validation entreprise ; ce canal peut rester en attente jusqu'à activation.",
    whatsappPlaceholder: "+33 6 xx xx xx xx",
    privacy:
      "Ce sont tes données personnelles, utilisées uniquement pour te notifier et jamais partagées.",
    saved: "Canaux enregistrés.",
    saveFail: "échec de l'enregistrement",
    disabledByAdmin: "Ce canal est actuellement désactivé. Il fonctionnera une fois réactivé.",
    notReady:
      "Ce canal n'est pas encore configuré sur le serveur. Il fonctionnera automatiquement une fois configuré.",
    telegramConnect: "Connecter Telegram (en un geste)",
  },

  quiet: {
    title: "Heures silencieuses",
    sub: "Pendant ces heures, les notifications arrivent en silence (sans son/alarme).",
    start: "Début",
    end: "Fin",
    decA11y: "Diminuer l'heure de {{label}}",
    incA11y: "Augmenter l'heure de {{label}}",
  },

  announcements: {
    title: "Annonces",
    empty: "Aucune annonce pour l'instant",
    emptyHint: "Les nouveautés et actualités apparaîtront ici.",
    pinned: "Épinglé",
  },
  settings: {
    account: "Votre compte Whenly",
    language: "Langue",
    appearance: "Apparence",
    themeSystem: "Système",
    themeLight: "Clair",
    themeDark: "Sombre",
    appearanceA11y: "Choisir l'apparence",
    languageA11y: "Choisissez la langue de l'application",
    supportTitle: "Support & Contact",
    supportSub: "Signalez un problème · support en direct · courrier",
    supportA11y: "Ouvrez le support et le contact",
    notifs: "notifications",
    notifsHint: "Accordez la permission et enregistrez cet appareil pour les notifications (FCM).",
    notifsBtn: "activez les notifications",
    notifsOk: "Appareil enregistré — notifications active.",
    notifsDevice: "Les notifications fonctionnent sur un appareil réel (pas d'émulateur).",
    notifsDenied: "La permission de notification a été refusée.",
    privacy: "Données & confidentialité",
    privacySub: "vos droits GDPR · comment vos données sont protégées",
    websiteTitle: "Site web",
    websiteSub: "Site du produit · cas d'''usage et comparatifs",
    adminConsole: "Whenly Console",
    adminConsoleSub: "Console d'administration · vous seul",
    privacyA11y: "Droits et confidentialité des données — page de support",
    danger: "zone de danger",
    dangerHint:
      "Supprimez définitivement votre compte et toutes vos données (GDPR). Cela ne peut pas être annulé.",
    deleteBtn: "supprimer le compte",
    deleteTitle: "Supprimer le compte",
    deleteMsg:
      "Tous vos watchers, appareils et abonnements seront supprimés définitivement. Cela ne peut pas être annulé.",
    deleteFail: "n'a pas pu supprimer",
    signOut: "Se déconnecter",
    footer: "Whenly · voir Support & Contact pour vos droits de confidentialité et de données",
  },
  legal: {
    privacyTitle: "Politique de confidentialité",
    termsTitle: "Conditions d'utilisation",
    termsSub: "Description du service · limites honnêtes · responsabilité",
    updated: "Version {{v}} · mise à jour : {{date}}",
    noticeEn:
      "La version faisant foi de ce document est en anglais ; il n'existe pas encore de traduction dans ta langue.",
    exportTitle: "Télécharger mes données",
    exportSub: "Récupère toutes tes données en JSON (portabilité RGPD)",
    exportOk: "Ton export de données est prêt.",
    exportFail: "échec de l'export",
  },
  support: {
    title: "Soutien & Contact",
    thread: "Support en direct",
    contact: "contact",
    emailTitle: "Courriel",
    emailA11y: "Envoyer un courriel : {{mail}}",
    live: "support en direct",
    liveHint: "L'ouverture d'une demande notifie instantanément l'administrateur; discutez ici.",
    liveStart: "Démarrer le support en direct",
    liveOpening: "Ouverture…",
    liveFirstMsg: "Bonjour, j'ai besoin de support en direct.",
    report: "signaler un problème",
    reportPlaceholder:
      "Décrivez brièvement le problème — ce que vous avez fait, ce que vous attendiez, et ce qui s'est passé.",
    reportA11y: "Description du problème",
    reportSent: "D'accord, merci ! Nous répondrons par courriel si nécessaire.",
    sendFail: "ne peut pas envoyer",
    sending: "Envoi…",
    ticketA11y: "Billet de support, {{status}}, {{date}}",
    you: "Vous",
    supportTeam: "Équipe de support",
    msgPlaceholder: "Écrivez un message…",
    msgA11y: "Écrivez un message de support",
  },
  detail: {
    insights: "Analyses",
    detectionRate: "Taux de détection",
    confidenceTrend: "Tendance de confiance (dans le temps)",
    confidenceTrendA11y: "Tendance de confiance, {{n}} vérifications, dernière {{last}}%",
    valueTrend: "Tendance {{kind}}",
    numericTrend: "Tendance de la valeur numérique",
    numericTrendA11y: "Tendance de la valeur numérique, {{n}} détections",
    title: "Recherche",
    detections: "detections",
    noDetections: "Pas de détections encore. La surveillance continue.",
    history: "vérifier l'historique",
    noHistory: "Pas de vérifications effectuées encore.",
    detected: "détecté",
    noChange: "pas de changement",
    confidence: "{{n}}% de confiance",
    repeatNote: "signalé {{n}} fois · premier {{date}}",
    convo: "conversation avec le modèle",
    toModel: "watcher → modèle",
    watching: "En train d'observer : “{{q}}”",
    results: "Résultats de recherche ({{n}}):",
    noResultsOld:
      "Cette vérification a été effectuée avant que les transcriptions de recherche ne soient enregistrées — seul la réponse du modèle est stockée.",
    noResultsList: "Recherche effectuée ({{sum}}) ; liste de résultats non stockée pour ce record.",
    noSummary: "pas de résumé",
    decision: "modèle → décision",
    hit: "DETECTÉ",
    miss: "Pas de détection",
    noReasoning: "pas de raisonnement enregistré",
    expandA11y: "Vérifiez le {{date}} — {{status}}. {{action}} recherche et raisonnement",
    show: "Afficher",
    hide: "Cacher",
  },
};
