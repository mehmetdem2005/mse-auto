...
type Msg =
  ...
  | { role: "assistant"; content: string; reasoning?: string; steps?: Step[]; streaming?: boolean }
  | { role: "panel"; panel: { type: string; data: any } };
...
export default function AgentBoard() {
  ...
  useEffect(() => {
    let on = true;
    const t = async () => {
      try { const r = await fetch("/api/agents", { cache: "no-store" }); if (on) setFeed(await r.json()); } catch {}
    };
    t(); const iv = setInterval(t, 3000); return () => { on = false; clearInterval(iv); };
  }, []);

  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [msgs, busy, streamingTool]);
  ...
  useEffect(() => { try { if (msgs.length) localStorage.setItem("cc_chat", JSON.stringify(msgs.slice(-40).map((m: any) => ({ ...m, streaming: false })))); } catch {} }, [msgs]);
  ...
  const ticks = useMemo(() => {
    const out: any[] = [];
    for (let k = 0; k < 72; k++) {
      const a = (k * 5) * Math.PI / 180, big = k % 6 === 0, r1 = big ? 46.5 : 47.6;
      out.push({ x1: 50 + r1 * Math.cos(a), y1: 50 + r1 * Math.sin(a), x2: 50 + 49 * Math.cos(a), y2: 50 + 49 * Math.sin(a), o: big ? 0.5 : 0.22 });
    }
    return out;
  }, []);
  ...
  async function transcribe(blob: Blob) {
    ...
    try {
      ...
      if (j.text) await handleUtterance(j.text);
      else if (onRef.current) setVState("listening");
  ...
  function startVoice() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      ...
      const newRec = () => {
        ...
        rec.onstop = () => {
          const had = hadSpeech; const blob = new Blob(chunks, { type: "audio/webm" });
          if (had) transcribe(blob);
          if (onRef.current && !speakingRef.current) newRec();
        };
        rec.start(); mediaRef.current = { stream, rec, ctx };
      };
      const loop = () => {
        if (!onRef.current) return;
        if (speakingRef.current) { requestAnimationFrame(loop); return; }
        an.getByteTimeDomainData(buf); let sum = 0; for (const v of buf) { const x = (v - 128) / 128; sum += x * x; }
        const rms = Math.sqrt(sum / buf.length);
        if (rms > 0.055) { speechFrames++; if (speechFrames > 8) hadSpeech = true; silence = 0; }
        else if (hadSpeech) { silence += 1; if (silence > 55 && rec && rec.state === "recording") rec.stop(); }
  ...
  async function converse(text: string, init = false) {
    ...
    const history = init
      ? []
      : [...msgs.filter((m): m is Extract<Msg, { role: "user" | "assistant" }> => m.role !== "panel"), { role: "user" as const, content: text }];
    if (!init && text) setMsgs((m) => [...m, { role: "user", content: text }]);
    setBusy(true); setHeard(""); setStreamingTool(null);

    // Add placeholder assistant message with a unique ID so we can update it during streaming
    const sid = `s-${Date.now()}`;
    streamingIdRef.current = sid;
    setMsgs((m) => [...m, { role: "assistant", content: "…", steps: [], reasoning: "", streaming: true, _sid: sid } as any]);

    const updateMsg = (patch: Partial<{ content: string; reasoning: string; steps: Step[]; streaming: boolean }>) => {
      setMsgs((m) => {
        const idx = m.findIndex((x: any) => x._sid === sid);
        if (idx < 0) return m;
        const updated = [...m];
        updated[idx] = { ...updated[idx], ...patch } as Msg;
        return updated;
      });
    };
    ...
    try {
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (ev.type === "thinking") {
              updateMsg({ content: "⋯ Düşünüyor…" });
            } else if (ev.type === "tool_start") {
              setStreamingTool(TOOL_LABEL[ev.tool] || ev.tool);
              updateMsg({ content: `⚙ ${TOOL_LABEL[ev.tool] || ev.tool}…` });
            } else if (ev.type === "step") {
              liveSteps.push({ tool: ev.tool, result: ev.result });
              setStreamingTool(null);
              updateMsg({ steps: [...liveSteps], content: liveSteps.length === 1 ? liveSteps[0].result : `${liveSteps.length} işlem tamamlandı.` });
            } else if (ev.type === "reasoning") {
              updateMsg({ reasoning: ev.content });
            } else if (ev.type === "panel") {
              setMsgs((m) => [...m, { role: "panel", panel: ev.panel }]);
            } else if (ev.type === "reply") {
              const reply = ev.content || "Tamam.";
              updateMsg({ content: reply, reasoning: ev.reasoning || "", steps: ev.steps || liveSteps, streaming: false });
              if (ev.panel?.type) setMsgs((m) => [...m, { role: "panel", panel: ev.panel }]);
              if (vState !== "off" || onRef.current) {
                setVState("speaking"); speakingRef.current = true;
                await speak(reply);
                speakingRef.current = false;
                if (onRef.current) setVState("listening"); else setVState("off");
              }
            }
          } catch {}
        }
      ...
      updateMsg({ content: "Bağlantı hatası.", streaming: false });
    } finally {
      setBusy(false); setStreamingTool(null);
    }
  }
    ...
    <>
      <div className="console-bar">
        ...
        <span className="row" style={{ gap: 10 }}>
          ...
          <button className="chip" style={{ padding: "5px 10px" }} title="Sohbeti temizle"
            onClick={() => { setMsgs([]); try { localStorage.removeItem("cc_chat"); } catch {}; greet(); }}>🗑</button>
      ...
      <div className="cc-chat">
        {msgs.map((m, i) => m.role === "panel" ? (
          <InlinePanel key={i} panel={(m as any).panel} />
        ) : (
          <div key={i} className={`bubble ${m.role}${(m as any).streaming ? " streaming" : ""}`}>
            {(m as any).reasoning ? (
            ...
            {(m as any).steps?.length ? (
              <div className="steps">
                {(m as any).steps.map((st: Step, j: number) => (
                  <div key={j} className="step">✓ {TOOL_LABEL[st.tool] || st.tool} — {st.result}</div>
                ))}
              </div>
            ) : null}
        ...
        {busy && !msgs.some((m: any) => m.streaming) && (
      ...
      <form className="chat-input" onSubmit={(e) => { e.preventDefault(); if (input.trim()) { const t = input.trim(); setInput(""); converse(t); } }}>
        <input value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="Yaz ya da 🎙 ile konuş: 'kuyruğu göster', 'yeni konu: ____', 'sorunları çöz', 'analizi aç'…" autoFocus />
    ...
    <div className="cc-panel">
      ...
      {type === "queue" && (Array.isArray(data) && data.length ? data.map((j: any) => (
        <div className="card" key={j.id} style={{ marginBottom: 8 }}>
          ...
          {j.script?.hook && <div style={{ fontSize: 14, marginTop: 4, opacity: .8 }}>{j.script.hook}</div>}
          <div className="tag" style={{ marginTop: 6 }}>{j.review?.decision ? `kurul: ${j.review.decision.verdict}` : j.stage} · <a href="/queue" style={{ color: "var(--accent)" }}>tam sayfada →</a></div>
        </div>
      )) : <div className="tag">Onay bekleyen yok.</div>)}
      {type === "agents" && (Array.isArray(data) && data.length ? (
        <div className="grid" style={{ gap: 6 }}>
          {data.map((a: any, i: number) => (
            <div className="row" key={i} style={{ justifyContent: "space-between" }}>
              <span className="mono" style={{ fontSize: 13 }}>{a.agent_id}</span>
              <span className={`badge ${a.status === "completed" ? "good" : a.status === "failed" ? "bad" : a.status === "running" ? "review" : ""}`}>{a.status}</span>
            </div>
          ))}
        </div>
      ) : <div className="tag">Ajan durumu yok.</div>)}
      {type === "knowledge" && (Array.isArray(data) && data.length ? (
        <table><tbody>
          {data.map((k: any, i: number) => (
            <tr key={i}>
              <td>{k.topic}</td>
              <td className="mono" style={{ color: "var(--muted)" }}>{k.source_title}</td>
              <td><span className={`badge ${k.verified ? "good" : "bad"}`}>{k.verified ? "✓" : "?"}</span></td>
            </tr>
          ))}
        </tbody></table>
      ) : <div className="tag">Bilgi tabanı boş.</div>)}
      {type === "memory" && (Array.isArray(data) && data.length ? data.slice(0, 12).map((m: any, i: number) => (
        <div className="card" key={i} style={{ padding: "8px 12px", marginBottom: 6 }}>
          <span className="tag">{m.kind}</span> {m.content}
        </div>
      )) : <div className="tag">Hafıza boş.</div>)}
      {type === "analytics" && (Array.isArray(data) && data.length ? (
        <table>
          <thead><tr><th>Başlık</th><th>İzlenme</th><th>Beğeni</th></tr></thead>
          <tbody>
            {data.map((r: any, i: number) => (
              <tr key={i}><td>{r.title || r.video_id}</td><td className="mono">{r.views}</td><td className="mono">{r.likes}</td></tr>
            ))}
          </tbody>
        </table>
      ) : <div className="tag">Henüz veri yok — video yayınlayınca dolacak.</div>)}
    </div>
  );
}
