  ...
  return (
    <div className="card" style={{ marginTop: 12, borderLeft: `3px solid ${color}` }}>
      <div className="row" style={{ justifyContent: "space-between", marginBottom: 8 }}>
        <span className="eyebrow">Editöryel kurul · {review.verdicts?.length ?? 0} denetçi · {review.rounds} tur{review.mode ? ` · ${review.mode}` : ""}</span>
        <span className="badge" style={{ color }}>{d.verdict} · {d.passCount}/{review.reviewerCount} · puan {d.score}</span>
      </div>
      {!!review.plan?.length && (
        <details style={{ marginBottom: 8 }}>
          <summary className="eyebrow" style={{ cursor: "pointer" }}>Ajan sürüsü · {review.plan.length} alt-görev{review.swarmTrace ? ` · ${review.swarmTrace.filter((t: any) => t.ok).length}/${review.swarmTrace.length} ok` : ""}</summary>
      ...
      {!!d.fixes?.length && (
        <details style={{ marginTop: 8 }}>
          <summary className="eyebrow" style={{ cursor: "pointer" }}>İstenen düzeltmeler ({d.fixes.length})</summary>
          <ul style={{ margin: "6px 0 0", paddingLeft: 18 }}>
